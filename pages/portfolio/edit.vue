  ...
  <div class="wrapper lg:px-[100px] xl:px-[200px]">
    ...
    >
      ...
      <div class="mt-[24px] text-right">
        <Button
          :loading="isLoadingEditPortfolio"
...
export default {
  ...
  methods: {
    async getPortfolioById() {
      this.isLoading = true;
      try {
        const getPortfolio = await this.$axios.get(
          `/portfolios/${this.portfolioId}`
        );
        if (getPortfolio.data) {
          this.portfolio = getPortfolio.data.data;
          this.name = this.portfolio.name;
          this.value = this.portfolio.value;
          const fileName = this.portfolio.iconName;
    ...
    async onSavePortfolio() {
      ...
      this.isLoadingEditPortfolio = true;
      try {
        ...
        await this.$axios
          .patch(`/portfolios/${this.portfolioId}`, formData, {
            headers: {
          ...
          .then((response) => {
            if (response.status == 201) {
              this.$snackbar.show({
                message: "Berhasil menyimpan portfolio",
                isSuccess: true,
              });
        ...
        this.$snackbar.show({
          message: "Gagal menyimpan portfolio",
          isSuccess: false,
        });
      }
      this.isLoadingEditPortfolio = false;
    },
